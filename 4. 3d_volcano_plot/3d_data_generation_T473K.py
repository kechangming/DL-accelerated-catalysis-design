...
#consider more initial conditions for obtaining more reasonable roots.
...
np.savetxt('newn_T473K.csv', save_matrix,delimiter=',')

# 
csv_file=csv.reader(open(r'newn_T473K.csv','r'))
matrix=[]
for i in csv_file:
    matrix.append(i)
...
matrix_1=[]
for i in matrix:
    if not float(i[3])==-100:
        matrix_1.append(i)
matrix_2=np.array(matrix_1)
for i in matrix:
    if float(i[3])<-90:
        grid_z0=griddata(matrix_2[:,:3],matrix_2[:,3],i[:3],method='nearest')
#        print([i[0],i[1],i[2],float(grid_z0)])
        matrix_1.append([i[0],i[1],i[2],float(grid_z0)])
f=open(r'newn_T473K_interp.csv','w')
for i in matrix_1:
    f.write(str(i[0])+", "+ str(i[1])+", "+ str(i[2])+", "+ str(i[3]) + "\n")

#np.savetxt('newn.csv', matrix_1,delimiter=',')
f.close()